...
yargs(hideBin(process.argv))
  .command(
    ...
    async (argv) => {
      ...
      const ctx: any = { runId, triggerId, now: new Date().toISOString(), body, query, result: {} };
      const startIds = getNextNodeIdsFromTrigger(wf, triggerId, 'main');
      if (!startIds.length) throw new Error('No connections from trigger');
